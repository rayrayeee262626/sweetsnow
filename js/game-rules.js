(function (root, factory) {
  ...
  function modeOrders(mode){return TILE_TYPES.map((_,order)=>order).filter(order=>mode!=="sanma"||order===0||order>=8);}
  function createWall(mode){let id=0;return TILE_TYPES.flatMap((type,order)=>mode==="sanma"&&order>=1&&order<=7?[]:Array.from({length:4},()=>({...type,order,id:id++})));}
  ...
  function getTenpaiWaits(tiles,kans=0,mode){if(!Array.isArray(tiles)||tiles.length!==13-kans*3)return[];return TILE_TYPES.flatMap((type,order)=>mode==="sanma"&&order>=1&&order<=7?[]:getWinningType([...tiles,{...type,order,id:-1}],kans)?[order]:[]);}
