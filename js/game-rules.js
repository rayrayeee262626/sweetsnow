(function (root, factory) {
  ...
  function modeOrders(mode){return TILE_TYPES.map((_,order)=>order).filter(order=>mode==="chinitsu"?order<9:mode!=="sanma"||order===0||order>=8);}
  function createWall(mode){let id=0;return TILE_TYPES.flatMap((type,order)=>!modeOrders(mode).includes(order)?[]:Array.from({length:mode==="chinitsu"?12:4},()=>({...type,order,id:id++})));}
  ...
  function getWinningType(tiles,kans=0,mode){if(!Array.isArray(tiles)||kans<0||kans>4||tiles.length!==14-kans*3)return null;const c=countsFromTiles(tiles);if(mode==="chinitsu"){if(kans!==0||tiles.some(tile=>tile.order<0||tile.order>=9))return null;if(c.every(n=>n%2===0))return"七対子";return isStandardHand(c)?"通常形":null;}if(c.some(n=>n>4))return null;if(kans===0&&isThirteenOrphans(c))return"国士無双";if(kans===0&&isSevenPairs(c))return"七対子";return isStandardHand(c,4-kans)?"通常形":null;}
  function getTenpaiWaits(tiles,kans=0,mode){if(!Array.isArray(tiles)||tiles.length!==13-kans*3)return[];return TILE_TYPES.flatMap((type,order)=>!modeOrders(mode).includes(order)?[]:getWinningType([...tiles,{...type,order,id:-1}],kans,mode)?[order]:[]);}
  ...
  function getKanOptions(state){if(state.mode==="chinitsu"||state.finished||state.pendingTenpai||!state.drawnTile||state.wall.length===0)return[];const c=countsFromTiles([...state.hand,state.drawnTile]);return c.flatMap((n,order)=>n===4?[order]:[]);}
  ...
  return{TILE_TYPES,ORPHAN_INDICES,modeOrders,createWall,shuffle,getWinningType,getTenpaiWaits,newGame,discardFromHand,discardDrawn,getKanOptions,declareKan,advanceAfterTenpai,usedTileCounts,usedTileCount,physicalTileCount};
});
